...
const recursionPattern = (int1, int2) => {
 ...
 let stopPoint;
    const arr=[]
let length;
return arr.push( recursion(int1,int2))

function recursion(int1,int2){

    if(arr.length === 0){

        stopPoint=int1;
return (arr.push(int1),recursion(int1-int2,int2))
        }
        ...
        else if (arr.length !== length && stopPoint !== int1)
        {int1=int1-int2;
            return arr.push(recursionPattern(int1,int2));
        }   
        
        if(int1 == stopPoint && arr.length > 1)
        return arr;
